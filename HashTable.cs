...
namespace libman
{
        ...
        where TKey : IComparable<TKey>
    {
        ...
        public KeyedHashTable()
            : this(DEFAULT_CAPACITY, DEFAULT_C)
        { }
        ...
        public KeyedHashTable(int capacity, int c)
        {
        ...
        public void Insert(TData data)
        {
            ...
            int h = Math.Abs(data.Key.GetHashCode() % capacity);
            int step;
            for (step = 0;
                (table[h].State == NodeState.Used) && (table[h].Key.CompareTo(data.Key) != 0);
                ++step, h = (h + c * step) % capacity) ;
            table[h].Info = data;
        ...
        {
            int h = Math.Abs(key.GetHashCode() % capacity);
            int step;
            for
                (step = 0;
